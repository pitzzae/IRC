...
/*   Updated: 2017/11/17 18:16:57 by gtorresa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_irc_parse_is_file_argreg(t_env *e, int cs)
{
	...
	if (e->fds[cs].tfile)
	{
		...
		e->fds[cs].tfile = NULL;
		e->fds[cs].tlen = 0;
		dprintf(7, "is_truck add prev data new len (%d)\n", e->fds[cs].r);
	}
}
...
static int	ft_irc_parse_is_file_mreply(t_env *e, t_file *f)
{
	...
	while (l)
	{
		...
		{
			lf->send = 1;
			dprintf(7, "ready for next chuck\n");
			return (1);
		}
		l = l->next;
	}
